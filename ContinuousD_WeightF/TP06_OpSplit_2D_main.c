...
int main(int argc, char **argv)
{
  ...
  double Dcrit = 0.025;                    // threshold for active cell model
  double d = 0;
  double *phi;                             // weighting function for membrane current flow
  ...
  /* precalculate weighting phi */
  for (n = 1; n <= N; n++)
      {
      d = (D[n] - Dcrit) * 1000.0;
      phi[n] = exp(d)/(1.0+exp(d)); // Hill function
      }
      ...
      for (n = 1; n <= N; n++)
        {
          ...
          /* integrate ODEs using Rush and Larsen scheme */
            ...
            if ((celltype[n] == 1) && (D[n] >= 0.025))
              dV = phi[n] * dtshort * calculate_TP06_current_OpSplit( U, dtshort, lookup, celltype[n], stimCurrent );
            else
              dV = 0.0;
